...
part 'telkom_event.dart';
part 'telkom_state.dart';

class TelkomBloc extends Bloc<TelkomEvent, BlocState> {
  final TelkomRepository repository;

  TelkomBloc(this.repository) : super(InitialState()) {
    on<InquiryTelkomEvent>((event, emit) async {
      emit(ShowLoadingState());
      try {
        var results = await repository.inquiryTelkom(event);
        emit(HideLoadingState());

        if (results.status == Constants.RC_SUKSES || results.status == Constants.RC_R00) {
          emit(InquiryTelkomSuccessState(results.data!));
    ...
    on<PaymentTelkomEvent>((event, emit) async {
      emit(ShowLoadingState());
      try {
        var results = await repository.paymentTelkom(event);
        emit(HideLoadingState());

        if (results.status == Constants.RC_SUKSES || results.status == Constants.RC_R00) {
          emit(PaymentTelkomSuccessState(results.data!));
        } else if (results.status == Constants.RC_07 || results.status == Constants.RC_09) {
          emit(SessionExpiredState(results.description));
        } else {
          emit(ErrorState(message: results.description!.replaceAll("|", "\n\n")));
  ...
  generatePaymentTelkomContent(TransactionResponseModel data) {
  ...
  generatePaymentSpeedyContent(TransactionResponseModel data) {
    List<String> col1 = data.customerData!.replaceAll("\$", "").replaceAll("'", "").trim().split("_");
